...
func grapple(delta):
	...
	player_falling = !player.is_on_floor()
	...
	if grappling == true:
		...
		if player.gravity > -20:
			player.gravity -= (19 * delta - 2*delta)
		elif player.gravity < -20:
			player.gravity = -20
		...
		Rope.rope_length = (grapple_point.distance_to(player.transform.origin) * 0.2)
		...
		if Rope != null:
			if is_instance_valid(Rope) and not rotated:
				Rope.queue_free()
			rope_added = false
		if grapple.rotation != Vector3(0,0,0) and rotated == false:
